...
void WebHandler::startWebinterface() {
    ...
    CROW_ROUTE(app, "/api/content/list").methods("GET"_method)([&](const crow::request&) {
    ...
    CROW_ROUTE(app, "/api/content/<string>").methods("GET"_method)([&](const crow::request&, std::string contentId) {
    ...
    CROW_ROUTE(app, "/api/content/<string>/cover").methods("GET"_method)([&](const crow::request&, crow::response& res, std::string contentId) {
        if(!isValidContentId(contentId)) {
    ...
    CROW_ROUTE(app, "/api/tags").methods("GET"_method)([&](const crow::request&) {
    ...
    CROW_ROUTE(app, "/api/tags/<string>").methods("GET"_method)([&](const crow::request&, std::string tag) {
        if(!isValidTag(tag)) {
    ...
    CROW_ROUTE(app, "/api/tags/<string>").methods("POST"_method)([&](const crow::request& req, std::string tag) {
        ...
        try {
            ...
            if(obj.tag.empty() || obj.name.empty() || obj.action.empty()) {
                return crow::response(crow::status::BAD_REQUEST, "Required fields are missing");
            }
    ...
    CROW_ROUTE(app, "/api/tags/<string>").methods("DELETE"_method)([&](const crow::request&, std::string tag) {
        if(!isValidTag(tag)) {
            return crow::response(crow::status::BAD_REQUEST, "Invalid tag format");
        }
    ...
    CROW_ROUTE(app, "/api/state").methods("GET"_method)([&](const crow::request&) {
        applicationState->UpdateBatteryState(batteryHandler->getBatteryPercentage(), batteryHandler->isBatteryCharging());
        MusicBox::StateData stateData = applicationState->GetState();
        nlohmann::json j = stateData;
        auto response = crow::response(j.dump());
        response.add_header("Content-Type", "application/json");
        return response;
    });

    CROW_ROUTE(app, "/api/tags/<string>/reset").methods("POST"_method)([&](const crow::request&, std::string tag) {
        if(!isValidTag(tag)) {
            return crow::response(crow::status::BAD_REQUEST, "Invalid tag format");
        }
        std::optional<MusicBox::TagData> tagData = tagHandler->GetTagData(tag);
    ...
    CROW_ROUTE(app, "/api/content/reload").methods("POST"_method)([&](const crow::request&) {
        contentHandler->emptyLibrary();
        contentHandler->readContentDirectory("content");
        return crow::response(crow::status::OK);
    });
    ...
    CROW_ROUTE(app, "/api/live").websocket()
      ...
      .onclose(
          [&](crow::websocket::connection& conn, const std::string&) {
        std::lock_guard<std::mutex> _(mutexConnectedClients);
...
void WebHandler::setBatteryHandler(BatteryHandler* bHandler) {
    batteryHandler = bHandler;
}
...
void WebHandler::handleWebsocketData (crow::websocket::connection& , const std::string& data, bool) {
    std::cout << "socker msg: " << data << std::endl;    
    if("startPlay" == data) {
        musicHandler->startPlay();
        applicationState->UpdatePlayState(true);
        sendMessage(nlohmann::json{{"event", "play"}}.dump());
    }
    if("stopPlay" == data) {
        musicHandler->stopPlay();
        applicationState->UpdatePlayState(false);
